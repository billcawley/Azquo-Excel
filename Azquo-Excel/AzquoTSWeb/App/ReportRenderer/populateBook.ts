function populateBook1(bindings: Office.AsyncResult) {
    ...
    //String context = "";
    // why a sheet loop at the outside, why not just run all the names? Need to have a think . . . REMOVE SHEET LOOP FOR JAVASCRIPT - WFC
    ...
    for (var i in bindings) {
        if (bindings.value[i].substring(0, 13).toLower() == 'az_dataregion') {
            ...
            var region: String = bindings.value[i].substring("az_dataregion".length);
            for (var range in bindings) {
                var bind: String = bindings.value[range];
                if (bind.length > region.length) {
                    var facet: string = bind.substring(bind.length - region.length);
