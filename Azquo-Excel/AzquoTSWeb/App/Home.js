...
(function () {
    ...
    function initAzquo() {
        ...
        function onBindingDataChanged(edcea) {
            showNotification("Change", "data changed in " + edcea.binding.id);
            return;
        }
        function populateBook(ctx, bindings) {
            var bindingString = '';
            for (var i in bindings.value) {
                var rangeName = bindings.value[i].id;
                if (rangeName.length > 13 && rangeName.substring(0, 13).toLowerCase() === 'az_dataregion') {
                    ...
                    var region = rangeName.substring("az_dataregion".length);
                    for (var range in bindings.value) {
                        var binding = bindings.value[range];
                        var bind = binding.id;
                        if (bind.length > region.length) {
                            var facet = bind.substring(0, bind.length - region.length).toLowerCase();
